...
describe('Utils Test Suite', function () {
  describe('getProjectData Folder Priority', function () {
    it('test: Nearest Folder Priority', function () {
      ...
      assert.equal('yuidoc-root', d.name, 'must use nearest yuidoc.json first');
    });

    it('test: Finds package.json', function () {
      const d = Y.getProjectData('input/folders2');
      assert.equal('yuidoc-two', d.name, 'used deep yuidoc.json');
      assert.equal('package-root', d.description, 'used shallow package.json');
    });

    it('test: Finds package.json in same folder as yuidoc.json', function () {
      const d = Y.getProjectData('input/folders3');
      assert.equal('yuidoc-root', d.name, 'used deep yuidoc.json');
      assert.equal('package-root', d.description, 'used deep package.json');
    });

    it('test: Ignores package.json in deeper folder than yuidoc.json', function () {
      const d = Y.getProjectData('input/folders4');
      assert.equal('yuidoc-one', d.name, 'used deep yuidoc.json');
      assert.equal(undefined, d.description, 'used deep package.json');
    });

    it('test: Must be breadth-first', function () {
      const d = Y.getProjectData('input/folders5');
      assert.equal('yuidoc-two', d.name, 'used wrong yuidoc.json');
      assert.equal('package-two', d.description, 'used wrong package.json');
    });
  });
  ...
  describe('validatePaths', function () {
    it('test: path globs', function () {
      ...
      assert.equal(Array.isArray(options.paths), true, 'Failed to set path');
      assert.equal(options.paths.length, 3, 'Failed to retrieve all path options');
    ...
    it('test: ignore paths', function () {
      ...
      assert.equal(Array.isArray(options.paths), true, 'paths are present');
      assert.equal(options.paths.length, 1, 'one path present');
      assert.equal(options.paths[0], 'a', 'path a is in paths');
    });
  });
  ...
  describe('produce valid web urls', function () {
    it('test: Add paths onto end in sequence', function () {
      ...
      assert.equal(url, 'foo/bar/baz', 'parts should be added in sequence');
    });

    it('test: normalises windows paths into web happy urls', function (){
      const url = Y.webpath('foo\\bar', 'baz');
      assert.equal(url, 'foo/bar/baz', '\\ should be normalised to /');
    });

    it('test: joins relative paths', function (){
      const url = Y.webpath('./foo/bar', './baz');
      assert.equal(url, 'foo/bar/baz', 'should join relative paths');
    });
  });
  ...
  describe('getDirs', function () {
    it('test: gets paths as array', function() {
        ...
        assert.equal(Array.isArray(dirs), true);
    ...
    it('test: ignores paths', function() {
        ...
        assert.equal(Array.isArray(dirs), true);
        assert.equal(dirs.length, 2);
        assert.equal(dirs.indexOf(pathPrefix + '.gitignore'), -1, 'does not contain file some-file');
        assert.equal(dirs.indexOf(pathPrefix + '/a/b'), -1, 'does not contain symlink /a/b');
        assert.equal(dirs.indexOf(pathPrefix + '/c'), -1, 'does not contain path /c');
